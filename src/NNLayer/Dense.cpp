...
namespace NNLayer {
    Dense::Dense(size_t output_size, double speed, double bias_mean, double bias_variance, double weight_variance) :
...
//        std::cout << "got here" << std::endl;
        ...
        for (size_t i = 0; i < this->output_size; i++) {
            bias_gradient[i] += current_value_gradient[i]*this->speed;
        }
        for (size_t i = 0; i < this->input_size; i++) {
            double in_node_grad = 0;
            size_t page = 0;
            for (size_t j = 0; j < this->output_size; j++) {
                weights_gradient[page+i] += preceding_input[i]*current_value_gradient[j]*this->speed;
