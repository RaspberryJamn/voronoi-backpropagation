...
void NeuralNetwork::AddLayer(NNLayerType type, size_t size) {
    SDL_assert(this->built == false);

    NNLayer new_layer = new NNLayer(type);
    new_layer->SetInputSize(this->layers.back().GetOutputSize());
    new_layer->SetOutputSize(size);
    new_layer->CalculateParameterSize();
    this->values_size += new_layer->GetOutputSize();
    this->parameters_size += new_layer->GetParameterSize();
}
...
void NeuralNetwork::Backward(double* label, size_t label_size) { // MSE
    ...
    double* active_vector = this->backward_values+this->values_size;
    ...
    std::for_each(this->layers.end(), this->layers.begin(), [&](NNLayer layer) {
        layer.Backward(&active_vector, &active_weights, &active_gradients);
    });
}
